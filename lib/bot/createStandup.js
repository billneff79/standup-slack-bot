...
function createStandup(rtmBot, bot, message) {
	...
	if (scheduled !== false) {
		models.Channel.findOrCreate({
			...
			log.verbose('Standup scheduled for ' + message.channel + ' at ' +
...
function attachListener(controller, rtmBot) {
	controller.hears([/^\s*(schedule|create|move)\s+(?:standup\s+)?(.*)/],['direct_mention', 'slash_command'], createStandup.bind(null, rtmBot));
