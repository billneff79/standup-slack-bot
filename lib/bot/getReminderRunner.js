...
function runReports(anytimeBot) {
	...
	models.Channel.findAll({
		...
		if (channels.length > 0) {
			log.verbose('Sending reminders for ' + channels.length + ' channel(s)');
			...
			_.each(channels, (channel) => {
				...
				let reminder = {
					text: `${audience} :hourglass: There's a standup in ${channel.reminderMinutes} minutes!  To submit your standup, do \`/${process.env.SLASH_COMMAND} interview\`, DM #${channel.name}, or add any emoji to this message and I'll DM you to get your standup info.`,
					attachments: [],
