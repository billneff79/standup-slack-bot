...
module.exports = function doInterview(bot, interviewChannel, channelMention, interviewUser, singleSection) {
	...
	if (userInterviewQueue[interviewUser].current === interviewChannel) {
		return bot.say({
			text: `You are already running an interview for ${channelMention}, so I'm ignoring your duplicate request for the interview for that room.`,
...
module.exports.flush = function() {
	for (let user of Object.keys(userInterviewQueue)) {
		...
		if (userInterviewQueue[user].reminderTimer) {
			clearTimeout(userInterviewQueue[user].reminderTimer);
			delete userInterviewQueue[user].reminderTimer;
		}
...
function handleQueuedItem(dequeuedObject, taskFinished) {
	...
	/**
	 * Call this intead of taskFinished() to make sure consistent cleanup occurs
	 */
	function finishQueuedItem() {
		userInterviewQueue[interviewUser].current = null;
		taskFinished();
	}

	const { bot, interviewUser, interviewChannel, channelMention, userInfoPromise, singleSection } = dequeuedObject;
	...
	models.Channel.findOne({
		...
		if (!channel) {
			bot.startPrivateConversation({ user: interviewUser }, (response, convo) => {
				convo.say(`There are no configured standups for ${channelMention}.  Aborting interview.`);
			});
			return finishQueuedItem();
		}
		...
		models.Standup.findAll({
				...
				// TODO: consider allowing multi-line responses
					...
					interviewSetup.then(() => {
						...
						const resetReminderTimer = () => {
							if (userInterviewQueue[interviewUser].reminderTimer) {
								clearTimeout(userInterviewQueue[interviewUser].reminderTimer);
							}
							userInterviewQueue[interviewUser].reminderTimer = setTimeout(() => {
								bot.say({
									attachments: [{
										color: '#FFBB00',
										fields: [{
						...
						convo.on('end',(convo) => {
							if (userInterviewQueue[interviewUser].reminderTimer) {
								clearTimeout(userInterviewQueue[interviewUser].reminderTimer);
								userInterviewQueue[interviewUser].reminderTimer = null;
							}
			...
			.catch(e => {
				log.error('Error getting last 7 days of standups for ', channel, e);
				bot.startPrivateConversation({ user: interviewUser }, (response, convo) => {
					convo.say(`There was an error getting information for the interview for ${channelMention}.  Aborting interview.  Error message: : ${e}`);
				});
				return finishQueuedItem();
			});
	});
